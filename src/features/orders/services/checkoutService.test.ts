// Ruta destino: src/features/orders/services/checkoutService.test.ts
...
const { iniciarPago } = vi.hoisted(() => ({ iniciarPago: vi.fn() }))
vi.mock('@/features/payments/services/paymentService', () => ({
    paymentService: { iniciarPago },
}))

const { crearPedido } = vi.hoisted(() => ({ crearPedido: vi.fn() }))
vi.mock('./orderService', () => ({
    orderService: { crearPedido },
}))

import { buildPedido, checkoutService } from './checkoutService'
...
describe('buildPedido', () => {
    it('lanza error si el carrito está vacío', () => {
        ...
        expect(() =>
            buildPedido({ direccionId: 1, despacho: 'NORMAL' as never, prioridad: 'NORMAL' as never }),
        ).toThrow('El carrito está vacío.')
    })

    it('arma el payload con tipo_venta fijo WEBPAY y los detalles del carrito', () => {
        toDetalles.mockReturnValue([{ producto_id: 10, cantidad: 2 }])

        const payload = buildPedido({
            direccionId: 5,
            despacho: 'EXPRESS' as never,
            prioridad: 'ALTA' as never,
        })
    ...
    it('recorta espacios en la observación', () => {
        ...
        const payload = buildPedido({
            direccionId: 1,
            despacho: 'NORMAL' as never,
            prioridad: 'NORMAL' as never,
...
describe('checkoutService (Facade)', () => {
    it('crearDesdeCarrito arma el pedido y delega en orderService.crearPedido', async () => {
        ...
        expect(crearPedido).toHaveBeenCalledWith(
            expect.objectContaining({ direccion_entrega_id: 1, tipo_venta: 'WEBPAY' }),
        )
